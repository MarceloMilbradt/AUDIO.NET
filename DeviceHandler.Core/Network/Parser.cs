...
namespace SmartLedKit.Core.Network
{
    internal static class Parser
    {
        ...
        internal static byte[] EncodeRequest(Command command, string json, byte[] key, ProtocolVersion protocolVersion = ProtocolVersion.V33)
        {
            ...
            if (protocolVersion == ProtocolVersion.V33)
            {
                ...
                if ((command != Command.DP_QUERY) && (command != Command.UPDATE_DPS))
        ...
        internal static LocalResponse DecodeResponse(byte[] data, byte[] key, ProtocolVersion protocolVersion = ProtocolVersion.V33)
        {
            ...
            // Command
            var command = (Command)BitConverter.ToUInt32(BigEndian(data.Skip(8).Take(4)).ToArray(), 0);
            // Return code
            ...
            if (data.Take(PROTOCOL_VERSION_BYTES_31.Length).SequenceEqual(PROTOCOL_VERSION_BYTES_31))
            {
                ...
                realVersion = ProtocolVersion.V31;
            }
            ...
            if (data.Take(PROTOCOL_VERSION_BYTES_33.Length).SequenceEqual(PROTOCOL_VERSION_BYTES_33))
            {
                ...
                realVersion = ProtocolVersion.V33;
            }

            if (realVersion == ProtocolVersion.V33)
            {
                data = Decrypt(data, key);
            }

            if (data.Length == 0)
                return new LocalResponse(command, returnCode, null);

            var json = Encoding.UTF8.GetString(data);
            if (!json.StartsWith("{") || !json.EndsWith("}"))
                throw new InvalidDataException($"Response is not JSON: {json}");
            return new LocalResponse(command, returnCode, json);
        }
    }
}
